# -*- coding: utf-8 -*-
"""
Created on Wed Jul  1 15:54:14 2020
...
os.chdir(r'C:\karagoz\01-RESEARCH\01-Projects\01-In_silico_modeling_of_Integrin_function\003-Ligand_competition_model\ligands_free\04_equal_IC_high_FC')
font = {'family' : 'normal',
        'weight' : 'bold',
        'size'   : 13}
...
r2.steadyState()
#  3.6755597646411965e-06 --> this is a good number!
 
print(r2.getSteadyStateValuesNamedArray())
#     [IF],        [IT],         [I],        [C3],         [i],      [T],      [C1],        [C2],      [F]
# [[ 0.0211855, 3.79798e-05, 1.65474e-10, 2.57479e-05, 3.30948e-09, 0.329936, 0.0143625, 4.61588e-08, 0.280064]]
#these are the steady state values.

print(r2.getRatesOfChange())
# [-1.74594561e-10  1.22055144e-11 -3.46944695e-18 -2.84217094e-14 3.46944695e-18  0.00000000e+00] these are the rates of change of the 6 reactions, all approaching 0 --> confirm steady state. 

#%%
# simulate for day18 initial conditions,
# store results in pandas DataFrame "result" : 
r2 = te.loada(Ant_str)
result = pd.DataFrame(r2.simulate(0, 0.000016 , 100 , ['time', 'i', 'I','F','T', 'IF', 'IT',  'C1', 'C2', 'C3']), columns=['time', 'inactive', 'active','F','T', 'F_bound', 'T_bound', 'IF_IFclustered', 'IT_ITclustered', 'IF_ITclustered'])
...
result_old = pd.DataFrame(r2.simulate(0, 0.000016 , 100 , ['time', 'i', 'I','F','T', 'IF', 'IT',  'C1', 'C2', 'C3']), columns=['time', 'inactive', 'active','F','T', 'F_bound', 'T_bound', 'IF_IFclustered', 'IT_ITclustered', 'IF_ITclustered'])
#create new column with total integrin amount at each time step
result = result.assign(Sum = result.inactive + result.active + result.F_bound + result.T_bound +2*result.IF_IFclustered+2*result.IT_ITclustered+2*result.IF_ITclustered, Experiment="day18")
...
result_old = result_old.assign(Sum = result_old.inactive + result_old.active + result_old.F_bound + result_old.T_bound +2*result_old.IF_IFclustered+2*result_old.IT_ITclustered+2*result_old.IF_ITclustered, Experiment="day25")
...
df2.to_csv('equalIC_highFC_simResults.csv', sep='\t', index=False)
