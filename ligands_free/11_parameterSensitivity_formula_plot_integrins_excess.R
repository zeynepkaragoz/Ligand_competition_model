...
setwd("C:/karagoz/01-RESEARCH/01-Projects/01-In_silico_modeling_of_Integrin_function/003-Ligand_competition_model/ligands_free/08_parameter_sensitivity/01_integrins_in_excess/")
...
# read file
steadyState_all <- read.csv("parameterSensitivity_steadyStates_all_integrin_excess.csv", header = TRUE, sep="\t")
steadyState_all <- steadyState_all[,2:11]
...
PS_values_all$mol_species <- factor(PS_values_all$mol_species, levels = c("inactive", "active", "F_bound", "W_bound", "IF_IFclustered","IW_IWclustered","IF_IWclustered","F","W"))
#ready to plot
...
molecule_names <- c(inactive ="Inactive", active ="Active", F_bound ="L1-bound", W_bound ="L2-bound", IF_IFclustered="L1-L1 cluster", IW_IWclustered ="L2-L2 cluster", IF_IWclustered ="L1-L2 cluster", F = "L1", W ="L2")
up_down_label = c(up ="Parameter increased 20%", down ="Parameter decreased 20%")
...
ggsave("figureS2_parameter_sensitivity_integrins_excess.pdf", 
       width = 18,
       height = 25,
       units = "cm")
